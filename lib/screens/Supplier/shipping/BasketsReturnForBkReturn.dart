...
class BasketsReturnForBkReturn extends StatefulWidget {
  ...
  @override
  State<BasketsReturnForBkReturn> createState() =>
      _BasketsReturnForBkReturnState();
}

class _BasketsReturnForBkReturnState extends State<BasketsReturnForBkReturn> {
  ...
  String dateNewF = '';
  TextEditingController txtBasketReturn = TextEditingController();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        bottomNavigationBar: Footer3Layout(
            ...
            navigated: () {
              print(GlobalParam.typeMenuCode);
              if (widget.typeMenuCode == "T001") {
                ...
                if ((int.tryParse(txtBasketReturn.text) ?? 0) > 0) {
                  for (int i = 0;
                      i < GlobalParam.deliveryBasketReq.length;
                      i++) {
                    // print(
                    //     '$i = ${oldBasketList[i]} :${GlobalParam.deliveryBasketReq[i].iQTY}');
                    var basketName = '';
                    ...
                    if (oldBasketList[i] == true) {
                      GlobalParam.deliveryBasketReq[i].cTYPE = 'CTS';
                      GlobalParam.deliveryBasketReq[i].cREFDOC = '';
                      if (GlobalParam.PoHDAndPoDT.length > 0) {
                        GlobalParam.deliveryBasketReq[i].cREFDOC =
    ...
    try {
      ...
      for (int i = 0; i < GlobalParam.deliveryHisBasket.length; i++) {
        ...
        AddBKRReq addReturn = AddBKRReq(
          cREFDOC: GlobalParam.deliveryPodtList.length > 0
  ...
  addBasketReturn(double value, GetBasketResp data) {
    ...
    for (int i = 0; i < GlobalParam.deliveryBasketReq.length; i++) {
      if (GlobalParam.deliveryBasketReq[i].cBASKCD == data.cBASKCD) {
        ...
        total += GlobalParam.deliveryBasketReq[i]?.iTOTAL ?? 0;
      }
    }
...
class RangeTextInputFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(
    ...
    TextEditingValue maxValue = TextEditingValue(text: '$max');
    print('$min $max');
