...
class _RecheckProductStockFilterState extends State<RecheckProductStockFilter> {
  ...
  void initState() {
    ...
    // print(GlobalParam.customerRecheckPROStockList.length);
  }
        ...
        child: InkWell(
          onTap: () {
            ...
            if (typeCodeNum != '') {
              ...
              for (int i = 0; i < GlobalParam.customerRecheckPROStockList.length; i++) {
                if (GlobalParam.customerRecheckPROStockList[i].cTYPE == typeCodeNum) {
                  GlobalParam.customerShowRecheckPROStockList
                      .add(GlobalParam.customerRecheckPROStockList[i]);

                  typeF.add(GlobalParam.customerRecheckPROStockList[i]);
                }
              }
              if (catCodeList.length > 0) {
                ...
                for (int i = 0;
                    i < GlobalParam.customerRecheckPROStockList.length;
                    i++) {
                  for (int j = 0; j < catF.length; j++) {
                    if (GlobalParam.customerRecheckPROStockList[i].cPRODCD ==
                            catF[j].cPRODCD &&
                        GlobalParam.customerShowRecheckPROStockList.length <
                            GlobalParam.customerRecheckPROStockList.length) {
                      // print(catF[j].cPRODNM );
                      GlobalParam.customerShowRecheckPROStockList
                          .add(GlobalParam.customerRecheckPROStockList[i]);
                    }
