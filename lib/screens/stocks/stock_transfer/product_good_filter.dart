...
class StockTransferGoodFilter extends StatefulWidget {
  final String? pageNumber;
  const StockTransferGoodFilter({Key? key, this.pageNumber}) : super(key: key);

  @override
  State<StockTransferGoodFilter> createState() =>
      _StockTransferGoodFilterState();
}

class _StockTransferGoodFilterState extends State<StockTransferGoodFilter> {
  ...
  void initState() {
    ...
    // print(GlobalParam.customerGoodProList.length);
  }
        ...
        child: InkWell(
          onTap: () {
            ...
            if (typeCodeNum != '') {
              ...
              for (int i = 0;
                  i < GlobalParam.customerGoodProList.length;
                  i++) {
                if (GlobalParam.customerGoodProList[i].cTYPE == typeCodeNum) {
                  GlobalParam.customerShowGoodProList
                      .add(GlobalParam.customerGoodProList[i]);

                  typeF.add(GlobalParam.customerGoodProList[i]);
                }
              }
              if (catCodeList.length > 0) {
                ...
                for (int i = 0;
                    ...
                    i++) {
                  for (int j = 0; j < catF.length; j++) {
                    if (GlobalParam.customerGoodProList[i].cPRODCD ==
                            catF[j].cPRODCD &&
                        GlobalParam.customerShowGoodProList.length <
                            GlobalParam.customerGoodProList.length) {
                      // print(catF[j].cPRODNM );
                      GlobalParam.customerShowGoodProList
                          .add(GlobalParam.customerGoodProList[i]);
                    }
            ...
            Navigator.of(context).push(MaterialPageRoute(
                builder: (context) => StockTransferReturnGoodPro(false)));
          },
          child: Container(
              // width: 96,
              child: Center(
