...
class StockTransferMissFilter extends StatefulWidget {
  final String? pageNumber;
  const StockTransferMissFilter({Key? key, this.pageNumber}) : super(key: key);

  @override
  State<StockTransferMissFilter> createState() =>
      _StockTransferMissFilterState();
}

class _StockTransferMissFilterState extends State<StockTransferMissFilter> {
  ...
  void initState() {
    ...
    // print(GlobalParam.customerReturnProList.length);
  }
        ...
        child: InkWell(
          onTap: () {
            ...
            if (typeCodeNum != '') {
              ...
              for (int i = 0;
                  i < GlobalParam.customerReturnProList.length;
                  i++) {
                if (GlobalParam.customerReturnProList[i].cTYPE == typeCodeNum) {
                  GlobalParam.customerShowReturnProList
                      .add(GlobalParam.customerReturnProList[i]);

                  typeF.add(GlobalParam.customerReturnProList[i]);
                }
              }
              if (catCodeList.length > 0) {
                ...
                for (int i = 0;
                    ...
                    i++) {
                  for (int j = 0; j < catF.length; j++) {
                    if (GlobalParam.customerReturnProList[i].cPRODCD ==
                            catF[j].cPRODCD &&
                        GlobalParam.customerShowReturnProList.length <
                            GlobalParam.customerReturnProList.length) {
                      // print(catF[j].cPRODNM );
                      GlobalParam.customerShowReturnProList
                          .add(GlobalParam.customerReturnProList[i]);
                    }
            ...
            Navigator.of(context).push(MaterialPageRoute(
                builder: (context) => StockTransferReturnMissPro(false)));
          },
          child: Container(
              // width: 96,
              child: Center(
